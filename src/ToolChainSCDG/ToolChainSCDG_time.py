...
class ToolChainSCDG:
    ...
    def build_scdg(self, args, is_fl=False):
        ...
        frame = pd.DataFrame(columns = ["expl_method", "family", "nameFileShort", "tot_syscalls", "tot_uniq_syscalls", "nb_instructions", "covered_blocks", "active", "pause", "ExcessLoop", "timeout", "elapsed_time"])
        frame.loc[len(frame.index)] = [self.expl_method, self.familly, nameFileShort, sum(self.call_sim.syscall_found.values()), 
                                        ...
                                        len(simgr.stashes["pause"]), len(simgr.stashes["ExcessLopp"]), self.timeout, elapsed_time]

        if (os.path.exists("output/eval_SCDG_time/measurements_time.csv")):
            frame.to_csv("output/eval_SCDG_time/measurements_time.csv", mode='a', index=False, header=False)
        else:
            frame.to_csv("output/eval_SCDG_time/measurements_time.csv", mode='a', index=False)
