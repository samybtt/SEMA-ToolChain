...
class Graph_Builder():
    ...
    def __init__(self,mapping=None,name=None,merge_call=True,comp_args=True,min_size=3,ignore_zero=True,odir=None,get_info=False,verbose=False):
        self.DISCARD = {'LoopBreaker','Dummy_call'}# Nodes used for debug purpose but not real syscall
        self.TAKE = {}
    ...
    def build_links(self,trace,graph,dico={}):
        #print("Building links between calls")
        ...
        for i in range(len(trace)):
            ...
            if (call['name'] not in self.DISCARD) and (call['name'] not in self.mapping):
            ...
            if not is_present:
                ...
                self.nodes[str(self.id)] = str(call['addr'])+'.'+call['name']+' '+self.args_to_strings(call['args'])
                ...
                if call['args'] :
                    for j in call['args']:
                        ...
                        if str(j) in dico and str(j) !=' ' and str(j) != '' and str(j) != 'None' and str(j) != '0':
                            self.create_link((str(self.id),arg_id),dico[str(j)],graph)
                            ...
                            try :
                                if str(j) == '' or str(j) == ' ' or str(j) == 'None' or (self.IGNORE_ZERO and int(str(j)) == 0) :
                                    pass
                                else:
                ...
                if call['ret'] != None and ret != 'symbolic' :
                    try :
                        if str(j) == '' or str(j) == ' ' or str(j) == 'None' or (self.IGNORE_ZERO and int(ret) == 0 ) :
                            pass
                        else:
                            if ret in dico :
    ...
    def create_link(self,node1,node_list,dot,lab_type='1'):
        for i in node_list:
            ...
            if check and (check2 not in self.tablink ) and (check3 not in self.tablink)and not selfie:
                dot.edge(str(i[0]),str(node1[0]),label='('+str(i[1])+'-->'+str(node1[1])+')')
                label = lab_type+'9'+str(i[1])+'9'+str(node1[1])
                self.tablink.append('e '+str(i[0])+' '+str(node1[0])+' '+label+'\n')
    ...
    def build_graph(self,SCDG,format_out="json"):
        ...
        if self.MERGE_CALL :
            ...
            for i in range(len(SCDG)):
                if self.verbose:                    
                    print('Using SCDG '+str(i+1)+' over '+str(len(SCDG)))
                ...
                if len(SCDG[i]) >= self.MIN_SIZE:
                    ...
                    if self.verbose:
                        print('The SCDG '+str(i)+' was too small, smaller than '+str(self.MIN_SIZE)+' calls.')
            ...
            for n in self.tabnode :
                if format_out == "json":
                    ...
                    newnode = {'id':id_node,'name':content['name'],'addr':node_name.split('.')[0],'args':arg_node}
            ...
            tab_node = []
            tab_link = []
            for i in range(min(len(SCDG), 10)):
                if len(SCDG[i]) >= self.MIN_SIZE:
                    ...
                    tab_node = tab_node + self.tabnode
                    tab_link =  tab_link + self.tablink
                    
                   # dot.save('test-output/disjoint_union'+str(i)+'.gv')
                    #self.id = 0
                    self.tabnode = [] 
                    self.tablink = []
            ...
            for l in tab_link :
                if format_out == "json":
                    tab_split = l.split(" ")
                    newlink = {"id1":tab_split[1],"id2":tab_split[2],"label":tab_split[3]}
                    json_content['graph_'+str(i)]['links'].append(newlink)
                else:
                    self.graph_file.write(l)    
            #dot.save('test-output/disjoint_union.gv')
            if format_out == 'json':
                json.dump(json_content,self.graph_file)
        self.graph_file.close()
        #dot.render('test-output/disjoint_union.gv', view=False) # really heavy could crash
        ...
        if name_node in self.existing_nodes:
            ...
            if not self.COMP_ARGS :
                ...
                if name_node not in self.current_trace_nodes:
                    self.current_trace_nodes[name_node]={'name': call['name'],'args': [call['args']]}
                    return True

            name = self.existing_nodes[name_node]['name']
            ...
            if name_node not in self.current_trace_nodes:
                ...
                if flag :
                    ...
                    self.current_trace_nodes[name_node]={'name': call['name'],'args': self.current_trace_nodes[name_node]['args']}
    ...
    def is_match(self,test_list,new_list):
        ...
        for i in range(len(test_list)):
            ...
            #if isinstance(test_list[i],claripy.ast.bv.BV) or isinstance(new_list[i],claripy.ast.bv.BV) :
            #    return False
